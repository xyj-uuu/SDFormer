...
class CreateDepthFromLiDAR(object):
    def __init__(self, point_cloud_range, grid_size, projective_filter=True,
            label_mapping="/data/B221000559-XYJ/project/stereoscene/semantickitti.yaml"):
        self.grid_size = np.array(grid_size)
        ...
        self.lidar_root = '/data/B221000559-XYJ/project/WM-Project/kitti360/'
        self.lidarseg_root ="/data/B221000559-XYJ/project/WM-Project/kitti360/preprocess/labels/"
    ...
    def __call__(self, results):
        ...
        results["raw_points"] = lidar_points
        lidar_points = torch.from_numpy(lidar_points[:, :3]).float()

        # loading lidarseg labels
        # lidarseg_filename = os.path.join(self.lidarseg_root,sequence,  frame_id.replace(".png", "_1_2.npy"))
        ...
        lidarseg = lidar_points[lidarseg_mask]#torch.cat((lidar_points, lidarseg), dim=1)[lidarseg_mask]
        ...
        # loading lidar points
        lidar_filename = os.path.join(self.lidar_root, "msnet3d_pseudo_lidar/", sequence, frame_id.replace(".png" , ".bin"))
        lidar_points = np.fromfile(lidar_filename, dtype=np.float32).reshape(-1, 4)
        lidar_points = torch.from_numpy(lidar_points[:, :3]).float()
