...
class CreateDepthFromOccupancy(object):
    ...
    def __call__(self, results):
        ...
        img_depth[depth_projected_points[:, 1].round().long(), depth_projected_points[:,
                                                               0].round().long()] = depth_projected_points[:, 2]
        ...
        list(results['img_inputs'])[
            0] = imgs, rots, trans, intrins, post_rots, post_trans, bda_rot, img_depth.unsqueeze(0), sensor2sensors
    ...
    def visualize_image_labels(self, img, img_depth, img_seg):
        ...
        depth_points = torch.stack(
            (depth_points[:, 1], depth_points[:, 0], img_depth[depth_points[:, 0], depth_points[:, 1]]), dim=1)
...
class CreateDepthFromLiDAR(object):
    def __init__(self, point_cloud_range, grid_size, projective_filter=True,
        ...
        self.lidarseg_root = "./lidar/lidarseg/dataset/sequences"
    ...
    def __call__(self, results):
        ...
        imgs, rots, trans, intrins, post_rots, post_trans, bda_rot, gt_depths, sensor2sensors, calib, filenamesl = \
        results['img_inputs'][0]
        tmp1 = [imgs, rots, trans, intrins, post_rots, post_trans, bda_rot, img_depth.unsqueeze(0), sensor2sensors,
                calib, filenamesl]
        ...
        depth_projected_points = depth_projected_points[depth_order]
        img_depth[depth_projected_points[:, 1].round().long(), depth_projected_points[:,
                                                               0].round().long()] = depth_projected_points[:,
                                                                                    2]  ## [384, 1280]点云深度
        ...
        imgs, rots, trans, intrins, post_rots, post_trans, bda_rot, gt_depths, sensor2sensors, calib, filenamesr = \
        results['img_inputs'][1]

        tmp2 = [imgs, rots, trans, intrins, post_rots, post_trans, bda_rot, img_depth.unsqueeze(0), sensor2sensors,
                calib, filenamesr]
