...
import time
import pdb
from ..SegmentAnything.sam_encoder import SAMEncoder
from ..SegmentAnything.segment_anything.build_sam import sam_model_registry
from ..SegmentAnything.segment_anything.predictor import SamPredictor
...
class BEVDepthOccupancy(BEVDepth):
    def __init__(self, 
        ...
        sam_checkpoint = "/data/B221000559-XYJ/project/SGN/pretrain/sam_vit_b_01ec64.pth"
        model_type = "vit_b"
        self.sam = sam_model_registry[model_type](checkpoint=sam_checkpoint).to(device='cuda')
        self.encoder = SamPredictor(self.sam)
    ...
    def bev_encoder(self, x, depth, temporal_voxel):  
        ...
        x = self.img_bev_encoder_backbone(x)  #(1,128,128,128,16)
        ...
        x = self.img_bev_encoder_neck(x, depth ,temporal_voxel)  #(1,384,128,128,16)
    ...
    def extract_img_feat(self, img, img_metas, gt, mode):
        ...
        img_left, img_right = img[0][0], img[1][0]  
        
        img_left_ref_feature = self.image_encoder( img_left )
        ...
        for i in range(img_left.shape[0]):
            img_size = F.interpolate(img[0][i], (310, 1024))  # torch.zeros(1,3,310,1024)
            h, w = img_size.shape[-2:]
            padh = 1024 - h
            padw = 1024 - w
            x_img = F.pad(img_size, (0, padw, 0, padh))
            
            x_img = [{"image": x_img}]
            y = self.encoder.set_image(img_size.squeeze(0))
            pred_seg = self.encoder.predict(y, point_labels=np.array([0]), multimask_output=True)
            mask_seg = pred_seg[2]
            pred_seg = F.interpolate(pred_seg[3], (1024, 1024))[:, :, :310, :]
            pred_seg = F.interpolate(pred_seg, (370, 1220))
            pred_seg = F.interpolate(pred_seg, (48, 160))
            sam_list.append(pred_seg)
            mask_list.append(mask_seg)

        x, depth, temporal_voxel = self.img_view_transformer([x] + geo_inputs + [x2] + geo_inputs2 + [calib]+ [img, img2], gt, mode, img[0], img2[0], filenamesl, filenamesr,sam_list,mask_list)
