...
class ProbNet(BaseModule):
    ...
    def __init__(
        ...
        self.mask_net = nn.Conv2d(mid_channels, 112, kernel_size=1, padding=0, stride=1)
...
class ResnetEncoderMatching(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, input_height, input_width,
        ...
        self.projector = Project3D(batch_size=self.num_depth_bins,
                                   height=self.matching_height,
                                   width=self.matching_width)
        self.prob = ProbNet(in_channels=128, with_centerness=True, bev_size=(96, 320))
    ...
    def forward(self, current_image, lookup_images, poses, K, invK, min_depth_bin=0, max_depth_bin=112 ):
        ...
        # batch_waped_feature = self.prob(torch.cat([current_feats, lookup_feats.squeeze(1)],dim=1))
...
class TransformerEncoder(nn.Module):
    def __init__(self, input_channels=64, output_channels=512, num_encoder_layers=6, num_heads=8, dim_feedforward=2048, dropout=0.1):
        super(TransformerEncoder, self).__init__()

        # Patch Embedding
        self.patch_embedding = nn.Conv2d(512, 128, kernel_size=4, stride=4)

        # Transformer Encoder
        encoder_layer = nn.TransformerEncoderLayer(d_model=128, nhead=num_heads, dim_feedforward=dim_feedforward, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_encoder_layers)

        # Final Projection
        self.projection = nn.Conv2d(128, output_channels, kernel_size=1)

    def forward(self, x):
        # Patch Embedding
        x = self.patch_embedding(x)  # [batch_size, 128, 48, 160]
        batch_size, _, height, width = x.size()

        # Flatten the spatial dimensions
        x = x.view(batch_size, 128, -1).permute(2, 0, 1)  # [seq_len, batch_size, 128]

        # Pass through Transformer Encoder
        output = self.transformer_encoder(x)

        # Final Projection
        output = output.permute(1, 2, 0)  # [batch_size, 128, seq_len]
        output = self.projection(output.view(batch_size, 128, height, width))
...
class ResnetEncoder(nn.Module):
    ...
    def forward(self, input_image):
        ...
        self.features.append(F.interpolate(self.trans(self.features[-1]),size=[12,40],mode='bilinear',align_corners=True))
